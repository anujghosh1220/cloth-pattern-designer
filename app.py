...
class CustomerFormLink(db.Model):
    __tablename__ = 'customer_form_link'
    
    id = db.Column(db.Integer, primary_key=True)
    measurement_id = db.Column(db.Integer, db.ForeignKey('saved_measurement.id'), nullable=False)
    ...
    # Relationships
    measurement = db.relationship('SavedMeasurement', backref='form_links')
...
def get_measurement_json(measurement_id):
    ...
    subquery = db.session.query(
        SavedMeasurement.id,
        db.func.row_number().over(
            order_by=SavedMeasurement.created_at.desc()
        ).label('row_num')
    ).filter(
        SavedMeasurement.user_id == current_user.id
    ).subquery()
    ...
    measurement_dict = {
        ...
        'job_number': f"#{measurement.job_number}",
...
@app.route('/measurement/<int:measurement_id>/generate_form_link', methods=['POST'])
@login_required
def generate_form_link(measurement_id):
    try:
        # Get the measurement and verify ownership
        measurement = SavedMeasurement.query.filter_by(
            id=measurement_id, 
            user_id=current_user.id
        ).first_or_404()
        ...
        form_url = url_for('customer_measurement_form', token=unique_token, _external=True)
...
@app.route('/customer-form/<token>')
def customer_measurement_form(token):
    try:
        # Find the form link
        form_link = CustomerFormLink.query.filter_by(
            unique_token=token,
            is_active=True
        ).first_or_404()
        
        # Check if link has expired
        if form_link.expires_at and form_link.expires_at < datetime.utcnow():
            return render_template('form_expired.html'), 410
        
        # Get the measurement with customer details
        measurement = SavedMeasurement.query.get(form_link.measurement_id)
        if not measurement:
            return render_template('form_expired.html'), 404
        ...
        app.logger.error(f"Error loading customer form: {str(e)}")
        return render_template('form_expired.html'), 500

@app.route('/customer-form/<token>/submit', methods=['POST'])
def submit_customer_measurement(token):
    try:
        # Find and validate the form link
        form_link = CustomerFormLink.query.filter_by(
            unique_token=token,
            is_active=True
        ).first_or_404()
        ...
        # Check if link has expired
        if form_link.expires_at and form_link.expires_at < datetime.utcnow():
            return jsonify({'error': 'Form link has expired'}), 410
        ...
        # Get category-specific measurements from form
        category = measurement.category.lower()
        
        # Only update measurements that are provided in the form with non-empty values
        if category == 'lehenga':
            hip_value = request.form.get('hip', '').strip()
            if hip_value:
                measurement.hip = float(hip_value)
            
            waist_floor_value = request.form.get('waist_floor', '').strip()
            if waist_floor_value:
                measurement.waist_floor = float(waist_floor_value)
            
            belt_value = request.form.get('belt', '').strip()
            if belt_value:
                measurement.belt = float(belt_value)
        elif category == 'blouse':
            length_value = request.form.get('length', '').strip()
            if length_value:
                measurement.length = float(length_value)
            
            chest_value = request.form.get('chest', '').strip()
            if chest_value:
                measurement.chest = float(chest_value)
            
            waist_value = request.form.get('waist', '').strip()
            if waist_value:
                measurement.waist = float(waist_value)
            
            front_neck_depth_value = request.form.get('front_neck_depth', '').strip()
            if front_neck_depth_value:
                measurement.front_neck_depth = float(front_neck_depth_value)
            
            back_neck_depth_value = request.form.get('back_neck_depth', '').strip()
            if back_neck_depth_value:
                measurement.back_neck_depth = float(back_neck_depth_value)
            
            sleeve_length_value = request.form.get('sleeve_length', '').strip()
            if sleeve_length_value:
                measurement.sleeve_length = float(sleeve_length_value)
            
            armhole_value = request.form.get('armhole', '').strip()
            if armhole_value:
                measurement.armhole = float(armhole_value)
            
            biceps_value = request.form.get('biceps', '').strip()
            if biceps_value:
                measurement.biceps = float(biceps_value)
            
            sleeve_cuff_value = request.form.get('sleeve_cuff', '').strip()
            if sleeve_cuff_value:
                measurement.sleeve_cuff = float(sleeve_cuff_value)
            
            shoulder_apex_value = request.form.get('shoulder_apex', '').strip()
            if shoulder_apex_value:
                measurement.shoulder_apex = float(shoulder_apex_value)
        elif category == 'kurti':
            length_value = request.form.get('length', '').strip()
            if length_value:
                measurement.length = float(length_value)
        ...
        measurement.updated_at = datetime.utcnow()
        measurement.notes = request.form.get('notes', measurement.notes)
        ...
        db.session.rollback()
        app.logger.error(f"Error submitting customer measurement: {str(e)}")
        return jsonify({'error': 'Failed to update measurements'}), 500
